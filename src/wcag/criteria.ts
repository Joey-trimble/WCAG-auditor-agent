...
const criteriaById = new Map(WCAG_22_CRITERIA.map((c) => [c.id, c]));

export function getCriterion(id: string): WcagCriterion | undefined {
  return criteriaById.get(id);
}

export function getCriteriaForTarget(version: WcagVersion, level: WcagLevel): WcagCriterion[] {
  const maxLevel = LEVEL_ORDER[level];

  return WCAG_22_CRITERIA.filter((c) => {
