...
import type { AuditFinding, AuditReport, Impact } from '../types';

type SarifLevel = 'error' | 'warning' | 'note' | 'none';

function impactToLevel(impact: Impact): SarifLevel {
  switch (impact) {
    case 'critical':
    case 'serious':
      return 'error';
...
function buildRuleDescriptor(finding: AuditFinding) {
...
export function buildSarifReport(report: AuditReport): object {
  const actionable = report.findings.filter((f) => !f.needsManualReview);
  const rules = new Map<string, ReturnType<typeof buildRuleDescriptor>>();
...
export function writeSarifReport(report: AuditReport, outputPath: string): void {
