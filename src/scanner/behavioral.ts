...
export async function runBehavioralChecks(
  ...
  const source = 'behavioral' as const;
...
async function checkMainLandmark(
  page: Page,
  config: AuditorConfig,
  ctx: Parameters<typeof createFinding>[1],
  findings: AuditFinding[],
...
async function checkPageHeading(
  page: Page,
  config: AuditorConfig,
  ctx: Parameters<typeof createFinding>[1],
  findings: AuditFinding[],
  ...
  if (skipInfo.focusableCount > 15) {
    findings.push(
      createFinding(config, ctx, {
        ...
        description:
          'No skip navigation link found and the page has many focusable elements. Users may need to tab through repetitive content (WCAG 2.4.1).',
        selector: 'body',
        criteria: ['2.4.1'],
        impact: 'moderate',
...
async function checkViewportZoom(
  page: Page,
  config: AuditorConfig,
  ctx: Parameters<typeof createFinding>[1],
  findings: AuditFinding[],
...
async function checkUnlabeledInputs(
  page: Page,
  config: AuditorConfig,
  ctx: Parameters<typeof createFinding>[1],
  findings: AuditFinding[],
...
async function checkFocusNotObscured(
  page: Page,
  config: AuditorConfig,
  ctx: Parameters<typeof createFinding>[1],
  findings: AuditFinding[],
  ...
  const obscured = await page.evaluate(() => {
    ...
    const target = (focusable[0] ?? document.body) as HTMLElement;
    target.focus();
    ...
    const obscurer = topEl as HTMLElement;
...
export async function extractNavSignature(page: Page, route: string): Promise<NavSignature> {
  ...
  for (const sig of withNav.slice(1)) {
    ...
    if (missing.length > 2 || extra.length > 2) {
      findings.push(
        createFinding(
          config,
          { route: sig.route, variant: 'default', source },
          {
