...
function renderFinding(f, checklist) {
    ...
    const hierarchy = checklistItem ? (0, hierarchy_1.formatCriterionHierarchy)(checklistItem) : undefined;
...
function renderFindingGroup(group, checklist) {
    const checklistItem = checklist.find((c) => c.id === group.criterionId);
    const hierarchy = checklistItem ? (0, hierarchy_1.formatCriterionHierarchy)(checklistItem) : group.criterionTitle ?? group.criterionId;
    return `
    <article class="finding" style="border:1px solid #e0e0e0;border-radius:8px;padding:16px;margin-bottom:12px">
      <header style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:8px">
...
function renderStaticByFile(violations) {
...
function writeHtmlReport(report, outputPath) {
    ...
    const wcag22Items = checklist.filter((c) => hierarchy_1.WCAG_22_ONLY_CRITERIA_IDS.includes(c.id));
